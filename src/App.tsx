...
const App: React.FC = () => {
  ...
  const toggleLanguage = () => {
    setLanguage((prevLang: string) => prevLang === 'en' ? 'es' : 'en');
  };
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      if(currentUser.role === 'barber') {
        setSelectedBarberForSchedule(currentUser.id);
      } else if (currentUser.role === 'admin' && barbers.length > 0) {
        setSelectedBarberForSchedule(barbers[0].id);
      }
    }
  }, [currentUser, barbers]);
  ...
  const fetchAppointments = useCallback(async () => {
    ...
    try {
      ...
      if (currentUser.role === 'admin') {
        fetchedAppointments = await apiGetAllAppointments();
      } else if (currentUser.role === 'barber') {
        fetchedAppointments = await apiGetBarberAppointments(currentUser.id);
      } else if (currentUser.role === 'client') {
        fetchedAppointments = await apiGetClientAppointments(currentUser.email);
      }
  ...
  useEffect(() => {
    ...
    if ((currentPage === 'barberSchedule' || currentPage === 'myAppointments')) {
      fetchAppointments();
    }
     if (currentPage === 'adminDashboard' && currentUser?.role === 'admin') {
      // For admin, the main data load already gets everything, but we can refresh if needed.
      // This ensures data is fresh if we navigate back to the dashboard.
      loadInitialData();
    }
  ...
  const handleCreateAppointment = async (appointmentData: Omit<Appointment, 'id' | 'status' | 'barberName' | 'serviceName' | 'price'> & {serviceId: string}) => {
    ...
    try {
      ...
      if (!selectedService || !selectedBarber) {
        throw new Error(getTranslation('selectedServiceOrBarberNotFound', language));
      }

      const newAppointment = await apiCreateAppointment({
        ...appointmentData,
  ...
  const handleDeleteBarber = async (barberId: string) => {
    ...
    setSuccessMessage(null);
    try {
      // Step 1: Delete the barber from the source of truth
      await apiDeleteBarber(barberId);
      
      // Step 2: Refetch all data to ensure UI is perfectly in sync
  ...
  const renderPage = () => {
    ...
    switch (currentPage) {
      case 'booking':
        return <BookingView services={services} barbers={barbers} onCreateAppointment={handleCreateAppointment} isLoading={isLoading} language={language} />;
      case 'myAppointments':
         return <ClientAppointmentsView appointments={appointments} isLoading={isLoading} onRefresh={fetchAppointments} language={language} />;
      case 'barberSchedule':
        return <BarberScheduleView 
                  barbers={barbers}
                  appointments={appointments}
                  selectedBarberId={selectedBarberForSchedule}
                  onSelectBarber={setSelectedBarberForSchedule}
                  isLoading={isLoading}
      ...
      case 'myProfile':
        if (currentUser.role === 'barber') {
          return <BarberProfileView 
                    currentUser={currentUser as Barber} 
